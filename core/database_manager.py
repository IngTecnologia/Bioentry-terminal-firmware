...
class DatabaseManager:
    ...
    async def _create_tables(self, db: aiosqlite.Connection) -> None:
        ...
        # Tabla de configuraci�n terminal
        await db.execute("""
    ...
    async def _configure_pragmas(self, db: aiosqlite.Connection) -> None:
        """Configura pragmas de SQLite para optimizaci�n"""
        pragmas = [
    ...
    async def get_user_by_document_id(self, document_id: str) -> Optional[Dict[str, Any]]:
        """Obtiene un usuario por su n�mero de documento"""
        try:
            async with self.get_connection() as db:
                sql = """
                    SELECT * FROM users 
    ...
    async def get_user_by_fingerprint_id(self, template_id: int) -> Optional[Dict[str, Any]]:
        """
        CR�TICO: Obtiene un usuario por su template_id del sensor AS608.
    ...
    # ==================== GESTI�N DE REGISTROS DE ACCESO ====================
    
    async def create_access_record(self, record_data: Dict[str, Any]) -> int:
        """
        CR�TICO: Crea un registro de acceso - SIEMPRE local primero.
        Esta es la funci�n m�s importante para el patr�n Local-First.
        """
        try:
            async with self.get_connection() as db:
                sql = """
    ...
    async def get_last_record_by_user(self, document_id: str) -> Optional[Dict[str, Any]]:
        """Obtiene el �ltimo registro de un usuario espec�fico"""
        try:
            async with self.get_connection() as db:
                sql = """
    ...
    async def increment_sync_attempts(self, record_id: int, error_message: Optional[str] = None) -> bool:
        ...
        try:
            async with self.get_connection() as db:
                ...
                if cursor.rowcount > 0:
                    self.logger.debug(
                        "Intentos de sincronizaci�n incrementados",
                        record_id=record_id,
    ...
    async def add_to_sync_queue(self, record_id: int, record_type: str, action: str, payload: Dict[str, Any]) -> int:
        ...
        try:
            async with self.get_connection() as db:
                ...
                self.logger.debug(
                    "Elemento a�adido a cola de sincronizaci�n",
                    queue_id=queue_id,
                    record_id=record_id,
    ...
    async def cleanup_old_records(self, days: int = 30) -> int:
        """Limpia registros antiguos seg�n la configuraci�n"""
        try:
    ...
    async def close(self) -> None:
        ...
        self.logger.info("Cerrando conexi�n a la base de datos")
        # En aiosqlite no hay conexi�n persistente que cerrar
        pass
...
if __name__ == "__main__":
    # Test b�sico del database manager
    async def test_database():
        ...
        # Test estad�sticas actualizadas
        stats = await db.get_database_stats()
